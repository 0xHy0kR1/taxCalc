...
document.addEventListener('DOMContentLoaded', function () {
    const incomeInputs = document.querySelectorAll('.gross-income-input, .add-income-input, .deductions-income-input');
    const submitButton = document.querySelector('.submit');

    incomeInputs.forEach(function(incomeInput) {
        incomeInput.addEventListener('input', function (event) {
            const inputValue = event.target.value;
            const alertIcons = event.target.parentElement.querySelector('.alert-icon'); 
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    submitButton.addEventListener('click', function(){
        ...
        if(!isAgeSelected()){
            ...
            console.log("Modal age not backdrop", overlay)
            // const modal = document.getElementById('exampleModal');
            console.log("Modal age not selected", modal);
            // Hide the modal
            modal.classList.remove("active");
            modal.classList.remove("display");
            overlay.classList.remove("display");
            ...
            const deductionsInput = document.querySelector('.deductions-income-input');
            console.log(deductionsInput)

            // Calculating the overall income after deductions
            let overallIncome = parseFloat(grossIncomeInput.value) + parseFloat(extraIncomeInput.value) - parseFloat(deductionsInput.value);
